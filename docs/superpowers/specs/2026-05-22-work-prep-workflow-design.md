...
- 로그인 작업자 드롭다운, 조장 선택 드롭다운, 같이 작업자 선택 목록에서 `조장` 표시는 흰색 단독 텍스트로 처리하지 않는다.
- 기본 select/dropdown 옵션은 흰색 배경이나 파란 선택 배경으로 렌더될 수 있으므로, 역할 텍스트는 일반 옵션 텍스트와 동일하게 어두운 글씨를 기본으로 둔다.
- 커스텀 배지로 표시할 경우에도 배경색과 글자색의 대비를 확보한다. 예: 연한 녹색 배경에는 진한 녹색 글씨를 사용한다.
- 선택된 옵션의 OS/browser 기본 하이라이트 색상에서도 `조장` 텍스트가 읽혀야 한다.
