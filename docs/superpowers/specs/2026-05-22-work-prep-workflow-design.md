...
- 로그인 화면의 작업자 선택 목록은 커스텀 드롭다운으로 구현하고, `조장`은 브랜드 컬러 배지 안에 흰색 글씨로 표시한다.
- 조장 선택 드롭다운, 같이 작업자 선택 목록 등 로그인 외 화면에서 기본 select/dropdown을 쓰는 경우에는 흰색 단독 텍스트를 사용하지 않는다.
- 기본 select/dropdown 옵션은 흰색 배경이나 파란 선택 배경으로 렌더될 수 있으므로, 로그인 외 역할 텍스트는 일반 옵션 텍스트와 동일하게 어두운 글씨를 기본으로 둔다.
- 커스텀 배지로 표시할 경우에도 배경색과 글자색의 대비를 확보한다. 예: 연한 녹색 배경에는 진한 녹색 글씨를 사용한다.
- 선택된 옵션의 OS/browser 기본 하이라이트 색상에서도 `조장` 텍스트가 읽혀야 한다. 로그인 화면의 커스텀 배지는 OS 기본 option 렌더링에 의존하지 않는다.
