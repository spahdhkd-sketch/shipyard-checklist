...
expectMatch(screenViews, /model\.showPlaceField \? `<div class="field material-flow-field work-prep-register-place-field">[\s\S]*data-work-prep-field="placeId"/, "the administrator work-order registration form should reveal the full-width map place selector");
expectMatch(app, /showPlaceField:\s*state\.adminMode,/, "the map place selector should be shown on the actual administrator registration form regardless of route");
...
expectMatch(app, /showSiteSurveyField:\s*state\.adminMode,/, "the site survey checkbox should be shown on the actual administrator registration form regardless of route");
...
expectMatch(app, /const adminIssueRequirementsRequired = state\.adminMode;/, "administrator issue validation should apply on the actual registration form regardless of route");
expectMatch(app, /adminIssueRequirementsRequired && \(!draft\.placeId[\s\S]*workPrepPlace[\s\S]*adminIssueRequirementsRequired && draft\.siteSurveyDone !== true[\s\S]*workPrepSiteSurvey/, "administrator work-order issuance should focus the first unmet place or site survey requirement");
