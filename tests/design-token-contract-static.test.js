...
assert(/@media \(max-width: 700px\)[\s\S]*?\.work-prep-mobile-filter-bar\s*\{[\s\S]*?grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\);/.test(styles));
...
assert(analyticsStyles.includes(".analytics-v4-monthly:not([open]) > .analytics-v4-monthly-content"));
