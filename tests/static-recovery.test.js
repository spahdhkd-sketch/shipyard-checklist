...
[
  ...
  "supabase/migrations/20260624090000_worker_push_attempt_rate_limit.sql",
...
assert.match(html, /assets\/dist\/css\/styles-v2\.min\.css\?v=20260624-material-push-1/);
assert.match(html, /assets\/dist\/css\/20-component-table\.min\.css\?v=20260624-material-push-1/);
assert.match(html, /assets\/dist\/css\/30-feature-signature\.min\.css\?v=20260624-material-push-1/);
assert.match(html, /assets\/dist\/css\/30-feature-push-management\.min\.css\?v=20260624-material-push-1/);
assert.match(html, /assets\/dist\/css\/30-feature-monthly-worker\.min\.css\?v=20260624-material-push-1/);
assert.match(html, /assets\/dist\/css\/20-component-disabled-reason\.min\.css\?v=20260624-material-push-1/);
assert.match(html, /assets\/js\/vendor\/supabase-js-2\.105\.3\.min\.js/);
assert.match(html, /assets\/dist\/js\/xlsx-helpers\.min\.js\?v=20260624-material-push-1/);
assert.match(html, /assets\/dist\/js\/analytics-model\.min\.js\?v=20260624-material-push-1/);
assert.match(html, /assets\/dist\/js\/ship-import-rules\.min\.js\?v=20260624-material-push-1/);
assert.match(html, /assets\/dist\/js\/push-rules\.min\.js\?v=20260624-material-push-1/);
assert.match(html, /assets\/dist\/js\/app-v2\.min\.js\?v=20260624-material-push-1/);
...
[
  ...
  assert.match(page, /assets\/dist\/css\/styles-v2\.min\.css\?v=20260624-material-push-1/, `${file} should use v2 styles with cache busting`);
  assert.match(page, /assets\/dist\/css\/20-component-table\.min\.css\?v=20260624-material-push-1/, `${file} should use table component styles with cache busting`);
  assert.match(page, /assets\/dist\/css\/30-feature-signature\.min\.css\?v=20260624-material-push-1/, `${file} should use signature feature styles with cache busting`);
  assert.match(page, /assets\/dist\/css\/30-feature-push-management\.min\.css\?v=20260624-material-push-1/, `${file} should use push management feature styles with cache busting`);
  assert.match(page, /assets\/dist\/css\/30-feature-monthly-worker\.min\.css\?v=20260624-material-push-1/, `${file} should use monthly worker feature styles with cache busting`);
  assert.match(page, /assets\/dist\/css\/20-component-disabled-reason\.min\.css\?v=20260624-material-push-1/, `${file} should use disabled-reason component styles with cache busting`);
  assert.match(page, /assets\/js\/vendor\/supabase-js-2\.105\.3\.min\.js/, `${file} should use the local Supabase vendor bundle`);
  assert.match(page, /assets\/dist\/js\/push-rules\.min\.js\?v=20260624-material-push-1/, `${file} should load push rules before the v2 app runtime`);
  assert.match(page, /assets\/dist\/js\/app-v2\.min\.js\?v=20260624-material-push-1/, `${file} should use the v2 app runtime with cache busting`);
...
assert.match(notFound, /assets\/dist\/css\/styles-v2\.min\.css\?v=20260624-material-push-1/);
assert.match(notFound, /assets\/dist\/css\/30-feature-not-found\.min\.css\?v=20260624-material-push-1/);
assert.doesNotMatch(notFound, /assets\/css\/styles\.css/);
const redesignPreview = read("redesign-v2.html");
assert.match(redesignPreview, /assets\/dist\/css\/20-component-table\.min\.css\?v=20260624-material-push-1/);
assert.match(redesignPreview, /assets\/dist\/css\/30-feature-signature\.min\.css\?v=20260624-material-push-1/);
assert.match(redesignPreview, /assets\/dist\/css\/30-feature-push-management\.min\.css\?v=20260624-material-push-1/);
assert.match(redesignPreview, /assets\/dist\/css\/30-feature-monthly-worker\.min\.css\?v=20260624-material-push-1/);
assert.match(redesignPreview, /assets\/dist\/css\/20-component-disabled-reason\.min\.css\?v=20260624-material-push-1/);
...
assert.match(app, /const APP_VERSION = "1\.4-20260624-material-push"/);
...
assert.match(app, /REMOTE_TABLES\.forEach\(\(config\) => localStorage\.removeItem\(storeKey\(config\.key\)\)\)/);
...
assert.match(app, /if \(legacyStorage\) console\.info\("Legacy local cache cleared for current app version\."\)/);
...
assert.match(app, /const pullConfigs = REMOTE_TABLES\.filter\(\(config\) => config\.pullOnStartup !== false\)/);
...
assert.match(app, /async function notifyMissingMaterialRegistered\(row\)/);
assert.match(app, /const PUSH_VAPID_PUBLIC_KEY = "/);
assert.match(app, /const MISSING_MATERIAL_PUSH_TARGET_NAMES = \["허지원", "김준혁", "김경제"\]/);
assert.match(app, /function unsafePushTargetWorkerIds\(\)/);
assert.match(app, /function missingMaterialPushTargetWorkerIds\(\) \{\s*return workerIdsForNames\(MISSING_MATERIAL_PUSH_TARGET_NAMES\);/);
...
assert.match(pushRules, /missingMaterial: \{[\s\S]*body: "\{호선\} · \{등록자\} · \{자재\} \{수량\}"/);
...
assert.match(app, /function unsafePushTargetWorkerIds\(\)/);
assert.match(app, /function missingMaterialPushTargetWorkerIds\(\)/);
...
assert.match(app, /pushNotificationFromTemplate\("missingMaterial", \{/);
assert.match(app, /sendWorkerPushNotification\(missingMaterialPushTargetWorkerIds\(\), \{/);
assert.match(app, /toast\("호선자재 누락이 접수되었습니다\."\);\s*notifyMissingMaterialRegistered\(row\);\s*syncMissingMaterial\(row\);/);
...
assert.match(app, /action: "verifyWorker"/);
...
assert.match(dashboardView, /data-stat-scope="unsafe" data-action="view-unsafe-received"/);
assert.match(dashboardView, /unsafeCount \? "즉시 확인" : ""/);
assert.doesNotMatch(dashboardView, /unsafeCount \? "즉시 확인" : "접수 없음"/);
assert.match(dashboardView, /<div><span>완료율<\/span><strong>\$\{todayCompletion\}%<\/strong><\/div>/);
assert.match(dashboardView, /statPill\(".*?", deliverySoon, ".*?", "#f97316", "clock", deliverySoon \? ".*?" : "", "delivery"/);
assert.match(dashboardView, /data-stat-scope="\$\{esc\(scope\)\}" data-history-scope="\$\{esc\(scope\)\}"/);
assert.match(dashboardView, /data-stat-scope="materials" data-action="view-material-list"/);
assert.doesNotMatch(dashboardView, /data-stat-scope="unsafe" data-view="unsafe"/);
assert.doesNotMatch(dashboardView, /data-stat-scope="materials" data-view="materials"/);
assert.match(dashboardView, /stat-foot \$\{foot \? "" : "is-empty"\}/);
...
const sw = read("sw.js");
assert.match(sw, /const APP_VERSION = "1\.4-20260624-material-push"/);
assert.match(sw, /const ASSET_TOKEN = "20260624-material-push-1"/);
...
const workerPushRateLimitMigration = read("supabase/migrations/20260624090000_worker_push_attempt_rate_limit.sql");
...
assert.match(pushFunction, /const MISSING_MATERIAL_PUSH_TARGET_NAMES = new Set\(\["허지원", "김준혁", "김경제"\]\.map\(normalizedWorkerName\)\)/);
assert.match(pushFunction, /async function missingMaterialTargetWorkerIds\(\)/);
assert.match(pushFunction, /const \{ worker, error: workerError \} = await verifyWorkerPushCredential\(workerId, employeeNo\)/);
assert.match(pushFunction, /const WORKER_PUSH_MAX_FAILED_ATTEMPTS = 5/);
assert.match(pushFunction, /async function assertWorkerPushAttemptAllowed/);
assert.match(pushFunction, /await recordWorkerPushFailedAttempt\(bucketKey, workerId, attempt\.attempt\)/);
assert.match(pushFunction, /worker_push_rate_limited/);
assert.match(pushFunction, /sendKind === "pledgePending" \|\| sendKind === "adminManual"/);
assert.match(pushFunction, /if \(sendKind === "unsafeIssue"\)/);
assert.match(pushFunction, /if \(sendKind === "missingMaterial"\)/);
...
assert.match(workerPushRateLimitMigration, /create table if not exists public\.worker_push_attempts/i);
assert.match(workerPushRateLimitMigration, /revoke all on table public\.worker_push_attempts from public, anon, authenticated/i);
assert.match(workerPushRateLimitMigration, /create policy "deny browser worker push attempts"/i);
