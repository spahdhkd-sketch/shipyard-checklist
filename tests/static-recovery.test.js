...
assert.match(html, /assets\/dist\/css\/styles-v2\.min\.css\?v=20260902-v2-1/);
assert.match(html, /assets\/dist\/css\/20-component-table\.min\.css\?v=20260902-v2-1/);
assert.match(html, /assets\/dist\/css\/30-feature-signature\.min\.css\?v=20260902-v2-1/);
assert.match(html, /assets\/dist\/css\/30-feature-push-management\.min\.css\?v=20260902-v2-1/);
assert.match(html, /assets\/dist\/css\/30-feature-monthly-worker\.min\.css\?v=20260902-v2-1/);
assert.match(html, /assets\/dist\/css\/20-component-disabled-reason\.min\.css\?v=20260902-v2-1/);
assert.match(html, /assets\/js\/vendor\/supabase-js-2\.105\.3\.min\.js/);
assert.match(html, /assets\/dist\/js\/xlsx-helpers\.min\.js\?v=20260902-v2-1/);
assert.match(html, /assets\/dist\/js\/analytics-model\.min\.js\?v=20260902-v2-1/);
assert.match(html, /assets\/dist\/js\/ship-import-rules\.min\.js\?v=20260902-v2-1/);
assert.match(html, /assets\/dist\/js\/push-rules\.min\.js\?v=20260902-v2-1/);
assert.match(html, /assets\/dist\/js\/app-v2\.min\.js\?v=20260902-v2-1/);
RUNTIME_SHELL_PAGES.forEach((file) => {
  const page = read(file);
  const appOffset = page.indexOf("assets/dist/js/app-v2.min.js?v=20260902-v2-1");
  let previousOffset = -1;
  RUNTIME_MODULES.forEach((name) => {
    const runtimeAsset = `assets/dist/js/${name}.min.js?v=20260902-v2-1`;
...
[
  ...
  assert.match(page, /assets\/dist\/css\/styles-v2\.min\.css\?v=20260902-v2-1/, `${file} should use v2 styles with cache busting`);
  assert.match(page, /assets\/dist\/css\/20-component-table\.min\.css\?v=20260902-v2-1/, `${file} should use table component styles with cache busting`);
  assert.match(page, /assets\/dist\/css\/30-feature-signature\.min\.css\?v=20260902-v2-1/, `${file} should use signature feature styles with cache busting`);
  assert.match(page, /assets\/dist\/css\/30-feature-push-management\.min\.css\?v=20260902-v2-1/, `${file} should use push management feature styles with cache busting`);
  assert.match(page, /assets\/dist\/css\/30-feature-monthly-worker\.min\.css\?v=20260902-v2-1/, `${file} should use monthly worker feature styles with cache busting`);
  assert.match(page, /assets\/dist\/css\/20-component-disabled-reason\.min\.css\?v=20260902-v2-1/, `${file} should use disabled-reason component styles with cache busting`);
  assert.match(page, /assets\/js\/vendor\/supabase-js-2\.105\.3\.min\.js/, `${file} should use the local Supabase vendor bundle`);
  assert.match(page, /assets\/dist\/js\/push-rules\.min\.js\?v=20260902-v2-1/, `${file} should load push rules before the v2 app runtime`);
  assert.match(page, /assets\/dist\/js\/app-v2\.min\.js\?v=20260902-v2-1/, `${file} should use the v2 app runtime with cache busting`);
...
assert.match(notFound, /assets\/dist\/css\/styles-v2\.min\.css\?v=20260902-v2-1/);
assert.match(notFound, /assets\/dist\/css\/30-feature-not-found\.min\.css\?v=20260902-v2-1/);
assert.doesNotMatch(notFound, /assets\/css\/styles\.css/);
const redesignPreview = read("redesign-v2.html");
assert.match(redesignPreview, /assets\/dist\/css\/20-component-table\.min\.css\?v=20260902-v2-1/);
assert.match(redesignPreview, /assets\/dist\/css\/30-feature-signature\.min\.css\?v=20260902-v2-1/);
assert.match(redesignPreview, /assets\/dist\/css\/30-feature-push-management\.min\.css\?v=20260902-v2-1/);
assert.match(redesignPreview, /assets\/dist\/css\/30-feature-monthly-worker\.min\.css\?v=20260902-v2-1/);
assert.match(redesignPreview, /assets\/dist\/css\/20-component-disabled-reason\.min\.css\?v=20260902-v2-1/);
...
const materialsV4View = read("assets/js/materials-v4-view.js");
...
assert.match(app, /const APP_VERSION = "1\.14\.5-20260902-v1"/);
...
assert.match(dashboardView, /<main class="home-v4 home-dashboard" aria-labelledby="homeV4Title">/);
assert.match(dashboardView, /<h1 id="homeV4Title">안전 운영 대시보드<\/h1>/);
assert.doesNotMatch(dashboardView, /data-home-sync=/);
assert.match(dashboardView, /<section class="home-dashboard__kpis" aria-labelledby="homeDashboardKpiTitle">/);
assert.match(dashboardView, /작업 전 점검 실행 확인/);
assert.match(dashboardView, /위험성평가 실행 확인/);
assert.match(dashboardView, /불안전요소 등록 · 접수 \$\{esc\(unsafeCount\)\}건/);
assert.match(dashboardView, /자재누락 등록 · 미완료 \$\{esc\(openMaterials\)\}건/);
assert.match(dashboardView, /data-view="manage" data-manage-center-card="operations"/);
assert.doesNotMatch(dashboardView, /home-v4__management/);
...
assert.match(app, /manageCenterMobileSectionOpen: false/);
assert.match(app, /manageSectionOpen: state\.view === "manage" && state\.manageCenterMobileSectionOpen/);
assert.match(app, /data-action="back-manage-center-menu"/);
assert.match(app, /class="manage-center__mobile-menu"/);
assert.match(styles, /\.manage-center__mobile-section\.is-mobile-section-open/);
assert.match(styles, /\.manage-center__mobile-menu-row/);
...
assert.match(materialsV4View, /canEdit && selectedCount \? `<button data-action="bulk-material-status"/);
assert.match(app, /event\.target\.matches\("\[data-material-bulk-select\]"\)[\s\S]*?renderPreservingScroll\(\);/);
assert.match(screenViews, /class="work-prep-mobile-filter-bar"/);
assert.match(screenViews, /data-record-filter="workPrep:shipNo"/);
assert.match(screenViews, /data-record-filter="workPrep:status"/);
...
const sw = read("sw.js");
assert.match(sw, /const APP_VERSION = "1\.14\.5-20260902-v1"/);
assert.match(sw, /const ASSET_TOKEN = "20260902-v2-1"/);
